...
def domain_nearby_schools_distance(component_props: dict) -> list:
    """
    Obtain school names based on given target property info page
    ...
    if "schoolCatchment" in component_props:
        if "schools" in component_props["schoolCatchment"]:
            return list(
                s["distance"] for s in component_props["schoolCatchment"]["schools"]
            )
        else:
            return list()
...
def domain_property_info(data: dict) -> Union[DataFrame, None]:
    ...
    property_df["nearBySchools_distance"] = property_df.apply(
        lambda x: domain_nearby_schools_distance(component_props), axis=1
    )
